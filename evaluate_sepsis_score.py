...
'''
def evaluate_sepsis_score_cp(Y_testi, predicted_hours2sepsis,max_hours,len_stay):
    # ground truth
    labels = [1 if i < max_hours else 0 for i in Y_testi]
     
    if len(labels) != len(predicted_hours2sepsis):
        sys.exit("!!! Error: The length of the predicted hours2sepsis is not equal to the length of the labels!!!")
    
    predicted_Label = [1 if i < min(len_stay, max_hours) else 0 for i in predicted_hours2sepsis]
...
def evaluate_sepsis_score(label_directory, prediction_directory):
    ...
    hours2sepsis_header = 'PredictedHours2Sepsis'
    ...
    for k in range(num_files):
        ...
        probabilities = load_column(prediction_files[k], hours2sepsis_header, ',')
...
def compute_prediction_utility(labels, predictions, 
                               dt_early=-12, dt_optimal=-6, dt_late=3.0, max_u_tp=1, min_u_fn= -2, u_fp=-0.05, u_tn=0.01,max_hours=480,
                               check_errors=True):
    # Check inputs for errors.
    # labels = [1 if i < max_hours else 0 for i in Y_testi]
    ...
    # print(f'Computing prediction utilities')
    if check_errors:
        if len(predictions) != len(labels):
            raise Exception('Numbers of predictions and labels must be the same.')
    ...
    # u = m * t + b.
    m_1 = float(max_u_tp) / float(dt_optimal - dt_early) # 1/(-6-(-12)) = 1/6
    b_1 = -m_1 * dt_early # -1/6*12 = -2
    m_2 = float(-max_u_tp) / float(dt_late - dt_optimal) # -1/(3-(-6)) = -1/9
    b_2 = -m_2 * dt_late # 1/9*3 = -1/3
    m_3 = float(min_u_fn) / float(dt_late - dt_optimal) # -2/(3-(-6)) = -2/9
    b_3 = -m_3 * dt_optimal # 2/9*6 = -4/3
    ...
    # print(f'observerd utility: {np.sum(u)}')
    return np.sum(u)
