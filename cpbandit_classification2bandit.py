...
from sklearn.svm import SVR
from get_dataset import SepsisData
from offline_cmab import OfflineContextualBandit
...
class CPBandit:
    def __init__(self,   experts, num_test_sepsis_pat_win, num_train_sepsis_pat_win):
        ...
        # self.rewards = []
        self.num_test_sepsis_pat_win = num_test_sepsis_pat_win
        self.num_train_sepsis_pat_win = num_train_sepsis_pat_win
        # balanced training set
        self.num_train_nosepsis_pat_win = num_train_sepsis_pat_win
   
    def _start_game(self):
        start_time = time.time()
        # dataset = (contexts, actions, rewards, test_contexts, mean_test_rewards)

        # return dataset,self.train_wins,self.test_wins
        self.data, train_wins, test_wins  = SepsisData(num_train_sepsis_pat_win=self.num_train_sepsis_pat_win, num_train_nosepsis_pat_win=self.num_train_nosepsis_pat_win, num_test_sepsis_pat_win=self.num_test_sepsis_pat_win)
        sim = 0
        cmab = OfflineContextualBandit(*self.data.reset_data(sim))
        ...
        # Reshape actions to make it a 2D array with shape (n_samples, 1)
        actions_reshaped = actions.reshape(-1, 1)
        # Concatenate X_train with actions along the columns (axis=1)
        X_train  = np.concatenate((self.data.contexts, actions_reshaped), axis=1)
        Y_train = self.data.rewards[np.arange(len(actions)), actions].reshape(-1, 1)


        # X_predict = cmab.test_contexts
        ...
        # final_result_path = f'../cpbanditsepsis_experiements/no_refit_balanced_win{win_size}_updatedreward/Results_{str(self.num_test_sepsis_pat_win)}_{str(self.num_train_sepsis_pat_win)}_{expert_list}' 
        final_result_path = f'./running_results/no_refit_balanced_win{win_size}_cmab_reward_regret/Results_{str(self.num_test_sepsis_pat_win)}_{str(self.num_train_sepsis_pat_win)}_{expert_list}' 
        ...
        with open(final_result_path+'/execution_info.txt', 'w') as file:
            ...
            file.write(f'num_test_sepsis_pat_win = {self.num_test_sepsis_pat_win}\n')
...
def main(): 
    ...
    # parser.add_argument('--num_test_sepsis_pat_win', type=int, help='Number of testing set')
    ...
    # cpbanit_player = CPBandit(experts=args.experts_list, num_test_sepsis_pat_win=args.num_test_sepsis_pat_win, num_train_sepsis_pat_win=args.num_train_sepsis_pat_win, num_train_nosepsis_pat_win = args.num_train_nosepsis_pat_win)
    # cpbanit_player._start_game()


 
    experts = ['ridge', 'lasso', 'rf', 'xgb', 'enet', 'dct', 'svr', 'nn']
    cpbanit_player = CPBandit(experts= experts, num_test_sepsis_pat_win= 2, num_train_sepsis_pat_win=15)
    cpbanit_player._start_game()
